...
export default function NotificationBell({ notifications, ringing, onClearAll }) {
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative' }}>
      ...
      {open && (
        <>
          <div className="dropdown-backdrop" onClick={close} aria-hidden="true" />
            ...
            <div style={{ overflow: 'auto', flex: 1 }}>
              {!count && (
                <div className="text-muted" style={{ padding: 40, textAlign: 'center', fontSize: 13 }}>
                  <Bell size={22} strokeWidth={1.75} style={{ opacity: .4, marginBottom: 8 }} />
                  <div>Ogeysiis lama helin · No notifications.</div>
                </div>
              )}
              {notifications.map((n) => (
                <div key={n.id} style={{ display: 'flex', gap: 10, padding: '13px 16px', borderBottom: '1px solid var(--border-soft)' }}>
